    ...
    if (confirm('Are you sure you want to reset the order counter to 1?\n\nThis will affect new order numbers only.')) {
        try {
            ...
            if (result.success) {
                ...
                // ALSO update the current window.orderCounter immediately
                ...
                console.log('Order counter reset to 1. Current window.orderCounter:', window.orderCounter);
                console.log('localStorage posOrderCounter:', localStorage.getItem('posOrderCounter'));
            } else {
...
// Fixed updateNextOrderDisplay function
function updateNextOrderDisplay() {
    ...
    if (nextOrderElement && window.orderCounter) {
        // Make sure it shows 4 digits like ORD-0001
        const paddedNumber = window.orderCounter.toString().padStart(4, '0');
        nextOrderElement.textContent = `ORD-${paddedNumber}`;
        console.log('Updated display to:', `ORD-${paddedNumber}`);
    } else if (nextOrderElement) {
        nextOrderElement.textContent = 'ORD-0001';
        console.log('Set default display to ORD-0001');
    }
}
